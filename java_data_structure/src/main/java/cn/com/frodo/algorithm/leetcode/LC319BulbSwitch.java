 ...
 * 输出：1
 *
 * 思路：第N个灯是否亮一定是奇数次的操作才会保留灯，手动拆分会发现n的因数轮会影响灯的开关。比如6的因数是1、2、3、6 对应4次。
 * 而完全平方数的因数是奇数个，比如16的因数是1，2，4，8，16 对应5个。
 * 结论：找1～N之间存在多少个完全平方数。而这个数可以转化为sqrt(n), 因为1<=x^2<=n，得到 1<=x<=sqrt(n)
 *
 * @author frodoking
 * @ClassName: LC
 * @date 2022/3/22
 */
public class LC319BulbSwitch implements IAlgorithm {
    @Override
    public void exec() {

        Assert.assertEquals(1, bulbSwitch2(3));
        Assert.assertEquals(4, bulbSwitch2(20));
        Assert.assertEquals(4, bulbSwitch(20));
    }
